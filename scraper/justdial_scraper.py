...
"""JustDial Scraper + pytest-friendly tests (with anti-bot driver setup)"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("data/logs/scraper.log", mode='w', encoding='utf-8'),
...
class JustDialScraper:
    ...
    def get_listings(self):
        try:
            return self.driver.find_elements(By.CSS_SELECTOR, 'div.resultbox_info, div.store-details.sp-detail-card')
    ...
    def _extract_contact(self, listing):
        ...
        try:
            ...
            if contacts:
                return contacts
            try:
                show_elem = listing.find_element(By.XPATH, './/*[contains(text(), "Show Number") or contains(text(), "SHOW NUMBER")]')
            ...
            if show_elem:
                ...
                if contacts:
                    return contacts
                try:
                    modal = self.wait.until(EC.presence_of_element_located((By.CSS_SELECTOR, 'section.jd_modal, div.jd_modal_content')))
    ...
    def _extract_inline_contact_link(self, listing):
        try:
            # Click the "Show Number" button if present
            try:
                ...
                pass  # No button means the number is probably already visible
    
            # Wait for the contact link to appear dynamically
    ...
    def _extract_modal_contacts(self, modal_element):
        try:
            ...
            return [l.get_attribute('href').replace('tel:', '').strip() for l in links if l.get_attribute('href')]
        except Exception:
            return []
